...
func CompareJSONFile(local map[string]interface{}, remote map[string]interface{}) (result map[string]interface{}) {
    ...
    if local == nil {
        mapData["status"] = "Local Json doesn't exist"
        Logger(mapData)
    }
    if remote == nil {
        mapData["status"] = "Remote Json doesn't exist"
        Logger(mapData)
    }
...
func CopyFiles(src string, dst string) (err error) {
    ...
    if err != nil {
        logs.Error("CopyFiles Error copy: " + err.Error())
        return err
    }
    return nil
}
...
func CheckVersion(confpath string) (status bool, err error) {
    ...
    if err != nil {
        logs.Error("CheckVersion Error checking newVersion: " + err.Error())
        return true, err
    }

    localVersion, err := GetVersion(confpath + config.Versionfile)
    if err != nil {
        logs.Info("CheckVersion current.version doesn't exists: " + err.Error())
        return true, err
    }
...
func RemoveDownloadedFiles(service string) (err error) {
    ...
    if err != nil {
        logs.Error("RemoveDownloadedFiles Error Removing " + service + " path: " + err.Error())
        return err
    }
    switch service {
    case "owlhmaster":
        ...
        if err != nil {
            logs.Error("RemoveDownloadedFiles Error Removing downloaded file for " + service + ": " + err.Error())
            return err
        }
...
func FullCopyFile(src, dst string) (err error) {
    in, err := os.Open(src)
    if err != nil {
        logs.Error("FullCopyFile Error opening file " + src + ": " + err.Error())
        return err
    }
    defer in.Close()
    ...
    if err != nil {
        logs.Error("FullCopyFile Error creating destination " + dst + " file: " + err.Error())
        return err
    }
    defer func() {
    ...
    if err != nil {
        logs.Error("FullCopyFile Error Copying file in memory from " + src + " to " + dst + ": " + err.Error())
        return err
    }

    err = out.Sync()
    if err != nil {
        logs.Error("FullCopyFile Error Sync source file from " + src + ": " + err.Error())
        return err
    }

    si, err := os.Stat(src)
    if err != nil {
        logs.Error("FullCopyFile Error readding permission from " + src + ": " + err.Error())
        return err
    }

    err = os.Chmod(dst, si.Mode())
    if err != nil {
        logs.Error("FullCopyFile Error setting permission to " + dst + ": " + err.Error())
        return err
    }
...
func FullCopyDir(src string, dst string) (err error) {
    ...
    si, err := os.Stat(src)
    if err != nil {
        logs.Error("Source directory doesn't exist: " + err.Error())
        return err
    }
    if !si.IsDir() {
        logs.Error("Source is not a directory: " + err.Error())
        return err
    }

    err = os.MkdirAll(dst, si.Mode())
    if err != nil {
        logs.Error("MkDirAll error: " + err.Error())
    }

    entries, err := ioutil.ReadDir(src)
    if err != nil {
        logs.Error("ReadDir source dir error: " + err.Error())
        return err
    }
    ...
    for _, entry := range entries {
        ...
        if entry.IsDir() {
            ...
            if err != nil {
                logs.Error("FullCopyDir error copy dir from " + srcPath + " to " + dstPath + ": " + err.Error())
            }
        } else {
            err = FullCopyFile(srcPath, dstPath)
            if err != nil {
                logs.Error("FullCopyFile error copy file from " + srcPath + " to " + dstPath + ": " + err.Error())
            }
        }
    }
